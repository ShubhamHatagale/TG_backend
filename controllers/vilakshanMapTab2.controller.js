const VilakshanMap = require("../models/vilakshanMapTab2.model");
...
exports.getRecordsById = async (req, res, next) => {
  try {
    const Data = await VilakshanMap.findAll({
      where: { id: req.params.vilakshanMapTab2Id, is_deleted: "0" },
    });
    if (!Data) {
      return res.status(404).json({
        status: 404,
...
exports.updateRecords = async (req, res, next) => {
  ...
  try {
    const VilakshanMapdetails = await VilakshanMap.update(
      {
        features: req.body.features,
      }, { where: { email_id: req.params.vilakshanMapTab2Id } }
    );
