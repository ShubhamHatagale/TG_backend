...
exports.getRecordsById = async (req, res, next) => {
  try {
    const Data = await Process.findAll({
      where: { parent_process_id: req.params.processId, is_deleted: '0' }
    });
    if (!Data) {
      return res.status(404).json({
        status: 404,
...
exports.getRecordsByProcessId = async (req, res, next) => {
  try {
    const Data = await Process.findAll({
      where: { id: req.params.processId, is_deleted: '0' }
    });
    if (!Data) {
      return res.status(404).json({
        status: 404,
...
exports.postRecords = async (req, res, next) => {
  try {
    const ProcessData = await Process.create({
      sr_no: req.body.sr_no,
      step_decription: req.body.step_decription,
      trasaction_time: req.body.trasaction_time,
      resource_allocated: req.body.resource_allocated,
      resource_name: req.body.resource_name,
      parent_process_id: req.body.parent_process_id,
...
exports.updateRecords = async (req, res, next) => {
  try {
    const ProcessData = await Process.update({
      sr_no: req.body.sr_no,
      step_decription: req.body.step_decription,
      trasaction_time: req.body.trasaction_time,
      resource_allocated: req.body.resource_allocated,
