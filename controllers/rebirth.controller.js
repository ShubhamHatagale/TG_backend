const Rebirth = require('../models/rebirth.model');
...
exports.getRecords = async (req, res, next) => {
  try {
    const Data = await Rebirth.findAll();
    if (!Data) {
...
exports.getRecordsById = async (req, res, next) => {
  try {
    const Data = await Rebirth.findAll({
      where: { email_id: req.params.rebirthId }
    });
    if (!Data) {
      return res.status(404).json({
        status: 404,
...
exports.postRecords = async (req, res, next) => {
  try {
    const Rebirths = await Rebirth.create({
      Search: req.body.Search,
      Manan: req.body.Manan,
      Sadhana: req.body.Sadhana,
...
exports.updateRecords = async (req, res, next) => {
  try {
    const Rebirthdetails = await Rebirth.update({
      Search: req.body.Search,
      Manan: req.body.Manan,
      Sadhana: req.body.Sadhana,
      Spiritual: req.body.Spiritual,
...
exports.deleteRecords = async (req, res, next) => {
  ...
  const rebirthId = req.params.id;
  try {
    const details = await Rebirth.update({
      is_deleted: '1'
    },
      { where: { id: rebirthId } });
    if (!details) {
